...
public class AdjacencyMatrixGraph {
    ...
    public boolean isBipartite() {
        color = new int[vertexAmount];

        for(int i = 0; i < vertexAmount; i++)
            color[i] = -1;

        return colorGraph(0, 1);
    }

    private boolean colorGraph(int p, int c) {
    ...
    public void printGroupedVertices() {
        if(isBipartite()) {
    ...
    public void printLinkedVertices() {
        for(int i = 0; i < vertexAmount; i++) {
            System.out.print("\n" + i + ": ");
            for(int j = 0; j < vertexAmount; j++) {
                if(adjMatrix[i][j] > 0)
